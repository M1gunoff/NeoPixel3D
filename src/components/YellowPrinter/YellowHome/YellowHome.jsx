...
const YellowHome = () => {
  ...
  return (
    <motion.div className="home__container" animate={controls} variants={pricingVariants} ref={ref} initial="hidden">
      <motion.h1 variants={containerVariants(1)}>
        Neopixel <motion.span variants={containerVariants(1.4)} className="home__titleBlue--yellow">Heliodorus</motion.span>
      </motion.h1>
      <motion.span variants={containerVariants(2)} className="home__description">
        <motion.p variants={containerVariants(2.5)}>
          Фотополимерный 3D принтер повышенной точности NeоPixel
        </motion.p>
        <motion.div variants={containerVariants(3)} className="home__button__wrapper">
          <a href="#contacts" className="home__button--yellow">Заказать</a>
        </motion.div>
      </motion.span>
    </motion.div>
  );
